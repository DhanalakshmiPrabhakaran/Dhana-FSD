...
async function getCountries() {
  ...
  for (countries of data) {
    ...
    if(countries.capital == undefined){
      div_6.innerHTML = `Capital: <span>Not Available</span>`;
    }
    else{
    ...
    const res2 = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?lat=${countries?.latlng[0]}&lon=${countries?.latlng[1]}&appid=7b656dbe8858bbac5322a600ff360839&units=metric`
    );
